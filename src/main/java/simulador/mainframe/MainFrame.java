...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("SIMULADOR DE AQUISIÇÃO DE DADOS PARA O SERVIDOR COM WEBSOCKETS");
        getContentPane().add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(15, 10, 570, -1));
        ...
        text_Porta_destino.setText("8080");
        ...
        jLabel4.setText("\"http://IP_DESTINO:PORTA_DESTINO/websocketfourrier/input\"");
        getContentPane().add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 240, -1, -1));

        jLabel6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        jLabel6.setText("com um campo chamado \"correntes\" que é um array com 4092");
        getContentPane().add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 280, -1, -1));

        jLabel7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        jLabel7.setText("valores variando entre +1 e -1, correspondentes a 32 ciclos.");
        ...
        jLabel9.setText("onde input é um servlet que recebe um objeto JSON chamado \"json\" ");
...
//        SystemInfo sys = new SystemInfo();
        ...
        while (ok) {
            ...
            tetaF += delta;
            enviarRequest(Float.toString(tetaF));
//System.out.println(sys.DiskInfo());
        }
    }
    
    void enviarRequest(String teta) {
        try {
        ...
        try {
            ...
            int fase = 50 + (int)Math.round(48D*Math.sin(ii/15D));
            
            dto.setCorrentes(construirJsonComCorrentes(fase));
            String dados = dto.toString();
            String request =
               "POST /websocketfourrier/input HTTP/1.1\r\n"
             + "Host: localhost:8080\r\n"
             + "Keep-Alive: 115\n"
             + "Content-Type: application/x-www-form-urlencoded\r\n"
             + "Content-Length: " + Integer.toString(dados.length() + 5) + "\r\n"
             + "\r\n"
             + "json="+dados;
...
//            System.out.print("=== FIM ===\n\n");
    ...
    private String[] construirJsonComCorrentes(int fase){
        String[] vStr = new String[4096];
        double[] vDouble = new double[4096];
        double x = 0D;
        ...
        for (int i=0; i<4096; i++) {
            x += deltax;
//            vDouble[i] = Math.sin((2D * Math.PI / 128) * x - 0.05D);
            vDouble[i] = Math.sin( (x/32) - 0.05D);
            // Se o sinal estiver proximo de zero
            if(Math.abs(vDouble[i])<eps){
                // reseta o contador
                k = 0;
            }
            // Se o contador for menor que a fase
            if(k<fase){
                k++;
                // zera o sinal
                vDouble[i] = 0D;
